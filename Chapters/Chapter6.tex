...
In this Chapter, the main clustering results from applying stream clustering using the data streams generated from both experiments are presented to discuss the strengths and weaknesses of the proposed DSAP algorithm. %of AP and discuss how DSAP provides potential solutions to these.
...
The streaming cluster analysis was aimed at finding any evolutionary patterns in stair usage due to an educational intervention campaign. The clustering results were generated using three months of the experiment, which were named as before, during, and after intervention.
...
The evolution of micro-clusters was interesting to be analyzed in order to interpret the results produced by clustering the data points accumulated for each landmark time window. In this Section we discuss the results generated on Wednesday May 1 that were observed between 7 am to 7 pm. Figure \ref{APhourly} shows the centroids of the micro-clusters and their respective data points that are connected using straight lines. The one hour expiration time was chosen to ensure that only the data points within the one hour window were used for finding clusters. 

The changes in stair usage across all levels can be clearly seen in Figure \ref{APhourly}. The micro-clusters show an increase in activity starting around 9-11 am. At the peak around noon, the micro-clusters become more dense occurring at different levels of the building. As the day progresses, the number of micro-clusters gradually decreases. These generated patterns were observed throughout the experiment, demonstrating the potential of using the DSAP approach for finding and tracking meaningful micro-clusters over space and time.
...
The clustering results are presented by visualizing all the centroids of the micro-clusters as blue circles while the centroids of the macro-clusters are represented as red crosses. The data points are visualized as black dots. The numbers of data points associated with each micro-cluster is mentioned right next to it. Searching for meaningful clusters have been achieved by evaluating weekly and monthly patterns.

\textbf{Weekly Patterns}

The first week of each of the three months under study were selected to interpret the clustering results. The first week of 18 March - 24 March before the intervention, the first week of 29 April - 5 May during the intervention, and the first week after the removal of the educational aids was from 27 May - 2 June. The selected expiration time equals to 160 was used for generating weekly patterns that are summarized in Table \ref{mactab}. Five macro-clusters have been found for each week, having a variety of density values ranging from 11 up to 4520. This overall trend of high density clusters with low counts was consistently present in data. It is also interesting to point out that the same macro-cluster (4,1) has happened during the three weeks of the analysis, but having different number of micro-clusters containing a large number of data points. 
...
The expiration time value of 670 was used to compute the monthly patterns as shown in Figure \ref{dsap3mon}. 
...
The streaming cluster analysis was carried out to reveal occupancy patterns in different buildings at the Universitat Jaume I campus, in Valencia, Spain. The clustering results were generated from analyzing data streams gathered during one day of the experiment. The experiment lasted for six days from May 30 to June 20, but June 20 was selected for the streaming cluster analysis due to a vast number of data points that was collected as shown in Figure \ref{timeline}. The first five days from May 30 until June 12, data streams were only available for building 1, but on June 20, data streams were available for both buildings 2 and 3. Finally, the only day that all users were participating in the experiment was June 20.
...
The most relevant observations for this study were the location of the user (spaceID) and user's phone (phoneID). A 10-minute landmark time window was chosen to analyze the UJIIndoorLoc data. The assumption was that the time interval of 10 minutes would accumulate sufficient number of data points for computing the micro-clusters without having issues with overflowing the time-based repository. Mainly because in this experiment the number of data points every minute was approximately ??? rather than saying it was high. CAN YOU PROVIDE AN ESTIMATE???  

Figure \ref{nama} shows all the data points generated in the three buildings named as T1, TD and TC, all of which have four floors except TC that has five floors.
...
\begin{figure}[!h]
    ...
    \\[\smallskipamount]    
    \caption{(a) 2D view of the location of all data points according to their buildings. (b) 3D view of the l.}
    \label{nama}
\end{figure}
...
The distribution of the total number of measurements made by the phones is shown in Figure \ref{userphone}. Phones with phoneIDs 8 and 9 were used the most in the entire experiment. 
...
\begin{figure}[!h]
    ...
    \\[\smallskipamount]    
    \caption{Phones usage during this experiment.}
    \label{userphone}
\end{figure}
...
In Table \ref{bdd} the total numbers of measurements made in each building is shown. It can be clearly seen that the TC building has significantly more number of data point as compared with the other two buildings. This could be due to it's additional floor that required additional measurements or maybe most of the users were present in the third building. Also, the pie chart in Figure \ref{Pfloor} shows that almost all building floors were equally occupied by the users with the exception of level five, which as mentioned before, is only present in the TC building. The number of measurements made at each spaceID location is depicted in Figure \ref{spaceiduniq }. The spaceID provides a unique identification for the rooms, classes, lab, offices etc., has a number ranging from 0 to 732 for  buildings T1, TD, and TC. SpaceID from 0 to 255 belongs to building T1, numbers between 256 to 416 belong to building TD, and the rest of it (417 to 732) belong to building TC.
...
\begin{table}[]
\centering
\caption{Buildings data distribution during experiment for all days.}
\label{bdd}
\begin{tabular}{
...
\begin{figure}
    ...
    \caption{Occupancy measured at different floor levels of all 3 buildings. In the text, we need to explain that building 3 is the only one that has a 5th floor.}
    \label{Pfloor}
\end{figure}
...
\begin{figure}
    ...
    \caption{spaceid data distribution during the entire experiment. Each color related to one building.}
    \label{spaceiduniq }
\end{figure}
...
The strength of DSAP lies in its speed of execution and the ability to analyze data as they arrive in a stream. Any concept drift present in the data is accurately identified in the micro-clustering phase, and the algorithm responds quickly to any change in the dataset.  In this Section, we discuss the results generated on June 20, the last day of the experiment. As part of this experiment, the limited number of users moved across several locations; hence it is important that the algorithm is able to find representative micro-clusters that reflect the change in the users' position. This process can be seen in action in Figure. \ref{drift} for a 10-minute window with an expiration time of one window. It is seen that rarely do two or more users occupy the same physical space hence the micro-clusters here just provide the summary of the movement of any active users within the current time window. It is obvious that DSAP can detect and track people's movements showing micro-clusters over time.
...
\subsection{Generated Micro and Macro-clusters}
The DSAP clustering algorithm is applied for the UJIIndoorLoc data to find meaningful clusters out of one day and the entire experiment. Blue circles are represented by the micro-clusters, while macro-clusters are visualized by red crosses. The data points are depicted as grey dots. 

\textbf{The June 20 Pattern}

The DSAP clustering result for the day June 20 is presented in Figure \ref{dsapjun20}. The second plot is a summary of this day. DSAP successfully finds the clusters for each building. It too finds clusters centered around 400, 500, and 659, but in addition, find another set of clusters around spaceID 300. It should be noted that the x and y axes are not of the same scale; hence even though the branches of the clusters might seem like that they are far apart, but they, in fact, belong to similar physical locations.
...
\begin{figure}[!h]
    \centering
    \includegraphics[width=.7\textwidth]{image/Chapters/Chapter6/DSAPJune20.png}
    \includegraphics[width=.7\textwidth]{image/Chapters/Chapter6/DSAPJune20Macro.png}
    \\[\smallskipamount]    
...
\subsection{Evaluation of Results}
To find hidden patterns in this data, first AP clustering algorithm was applied as a baseline to compare the performance and validation metrics. The computer used for this study could not handle the data from the whole experiment duration for clustering by AP, hence only one day of data, the 20 of June, is selected for this evaluation.


\textbf{Performance and Validation Metrics}
The results for DSAP and AP of each metrics along with the number of cluster, processing time and memory consumption are summarized in Table \ref{onedayevaluate}.
