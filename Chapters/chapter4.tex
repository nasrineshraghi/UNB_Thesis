...
% This window model is suitable for our model, since we are interested in the most recent data within the time frame. All data points within the window have the same weightage, which makes more sense for our hourly summary comparisons.
...
\todo[inline]{Is this algorithm in Chapter 5???}
...
\item[]\textbf{ActivateAP and Update  $\epsilon_{W_j}$}
  ...
  \todo[inline]{how to get rid of old centroids... as soon as the centroids are not longer significant to compute the E , they are disregarded using the damping factor. HOw do you do that??? } 
