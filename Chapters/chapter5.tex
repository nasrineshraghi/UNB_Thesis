...
\small
    ...
    \textbf{Input:}
         Data stream: $S$, landmark time window length: $L$ creates windows ${ W_0,W_1, ..., W_N : W_j = x_1, ..., x_L}$, start\_time;\newline 
    \textbf{Hyperparameters:}
        {Preference, Damping, $\epsilon$, ex}\newline
        \textbf{Output:}
        Set of micro clusters with centroids $Cm= [Cm_1,Cm_2,Cm_3, ...]$,  set of macro clusters with centroids $C_M= [C_{M_1}, C_{M_2}, ...C_{M_k}]$ \\
        
        \SetKwFunction{FIROC}{getWindowSize} 
         \SetKwProg{Fn}{Function}{}{}
         \Fn{\FIROC{start\_time, $L$} %\tcp{\color{green}return thewindow size} 
         }{
        ...
        \SetKwFunction{FIROC}{getAdaptiveThreshold}          
         \SetKwProg{Fn}{Function}{}{}
         \Fn{\FIROC{$W_j$, $Cm$}}{
         $\epsilon_T$ = mean(d($W_j, Cm$))
                  }
         \KwResult{$\epsilon_T$} %\newline
         \Statex  \hfill     \textbf{\textcolor{red}O\textcolor{red}n\textcolor{red}l\textcolor{red}i\textcolor{red}n\textcolor{red}e \textcolor{red}P\textcolor{red}h\textcolor{red}a\textcolor{red}s\textcolor{red}e\textcolor{red}:}\\
   	...
   	\SetKwFunction{FIROC}{AP}        \;  
         \SetKwProg{Fn}{Function}{}{}
         \Fn{\FIROC{ $W_0$}}{}
         \KwResult{Initial micro-clusters: $Cm = [Cm_1,Cm_2,...]$,\\ Cm_T = start\_time}\\
         $\epsilon_T$ = getAdaptiveThreshold($W_0, Cm$)\;
        %  Cm_T = start\_time
         }
         {
  $W_j$ = \texttt{S.getWindowSize(start\_time + L*j, L)}\;
      ...
      %  {$W_j = W_j + 1$}  \newline
        ...
        \eIf{min ($[Ed]) < \epsilon_T$}
        {   
        $Cm \leftarrow{x_i}   $
        (merge with micro\_clusters)\\%\tcp{kkkk}\\
        Cm_T = updated(start\_time)
        
        }
        {
        \STATE $cache \leftarrow{x_i} $ \hfill \Comment{Comparison}\;
        ...
        %\STATE $W_i \xrightarrow \mathrm{C_{m0}}$\;
         ...
         \SetKwProg{Fn}{Function}{}{}
         \Fn{\FIROC{Cache}}{}
         \KwResult{new micro-clusters:$[Cm']$}
      ...
      \If{$Cm_T < start\_time - ex * L$}
        ...
        %   \ContinuedFloat
        %   \hfill\textbf{{\textcolor{red}O\textcolor{red}f\textcolor{red}f\textcolor{red}l\textcolor{red}i\textcolor{red}n\textcolor{red}e \textcolor{red}P\textcolor{red}h\textcolor{red}a\textcolor{red}s\textcolor{red}e\textcolor{red}:}}\\
%          \If{j == N}{                        
...
Finally, the feature selection task was performed to provide a target dataset ready to be used by the simulation and two-phase clustering algorithm. Two numerical variables Position and Count were selected for analyzing the e-counter data.% and are illustrated in Table \ref{variab}
...
\subsection{WLAN Fingerprint-based Indoor Localization}
...
\begin{table}[!h]{}
\centering
\caption{ List of attributes of the UJIIndoorLoc dataset.}
\small
...
\begin{itemize}
    ...
    % \item For the streaming K-means algorithm $k$= 7 for each time window was selected based on the elbow method. The optimal k value was computed before the initiation of the stream clustering process.
