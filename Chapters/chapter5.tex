...
\caption{DSAP online Micro and offline Macro Clustering Algorithm}
    \label{algSDAPi}
\small
    \textbf{Input:}
      ...
      %  {$W_j = W_j + 1$}  \newline
        ...
        %  {                                  
         ...
         \KwResult{new micro-clusters:$[C'm]$\\
                $\epsilon_{W_J}'$ = getAdaptiveThreshold($C'm,cache$)\\
                $tq_{cache} = start\_time$
                }
        ...
        Updated timestamp $tq = tq + tq_{cache}$\\
